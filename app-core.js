...
const el={
  ...
  network:$('#networkStatus'),offline:$('#offlineBanner'),updateBanner:$('#updateBanner'),sourceReviewMeta:$('#sourceReviewMeta'),sourceProductMeta:$('#sourceProductMeta'),productVersionChip:$('#productVersionChip'),themeMeta:$('meta[name="theme-color"]')
};
...
function renderAppMeta(){const version=`V${APP_META.productVersion}`;if(el.productVersionChip)el.productVersionChip.textContent=version;if(el.sourceProductMeta)el.sourceProductMeta.textContent=`Uygulama: ${version}`;if(el.sourceReviewMeta)el.sourceReviewMeta.textContent=`Kaynak seti son gözden geçirme: ${formatDateTR(latestReviewDate())}`}
...
function openCase(id){
  ...
  const jumps=[['critical-actions','İlk adımlar','critical'],['algorithm','Algoritma',''],...(c.severity?[['severity','Şiddet','']]:[]),['red-flags','Acil uyarılar','critical'],...(c.meds?.length?[['medications','İlaçlar','']]:[]),['decision','Karar',''],['source','Kaynak','']];
  ...
  <div class="detail-body" style="${caseStyle(c)}">
    ...
    ${c.severity?`<section class="detail-section" id="severity"><div class="detail-heading"><span class="tiny-icon">3</span><div><h3>Klinik görünüm</h3><p>Bu ayrım hızlı eğitim görünümüdür; resmî arı sokması şemasında ayrı bir üçlü tedavi sınıflaması değildir.</p></div></div><div class="severity-tabs" role="tablist"><button type="button" role="tab" aria-selected="true" class="severity-tab active" data-level="mild">Lokal reaksiyon</button><button type="button" role="tab" aria-selected="false" class="severity-tab" data-level="moderate">Sistemik bulgu</button><button type="button" role="tab" aria-selected="false" class="severity-tab" data-level="severe">Anafilaksi</button></div>${renderSeverity(c)}</section>`:''}
